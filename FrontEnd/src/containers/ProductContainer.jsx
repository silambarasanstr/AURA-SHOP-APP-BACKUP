...
const ProductContainer = () => {
  ...
  // ✅ useCallback — handlers recreate ஆகாது
  const updateParams = useCallback(
    (updates) => {
      const next = { search: searchParam, category, page: pageParam, sort: sortParam, ...updates };
      const cleaned = {};
      Object.entries(next).forEach(([key, val]) => {
        const str = String(val);
        if (key === "page") cleaned[key] = str;
        else if (str !== "" && str !== "null" && str !== "undefined") cleaned[key] = str;
      });
