    ...
    <div className="flex flex-col gap-0.5">
      <button
        ...
        className={`flex items-center justify-between px-3 py-2 text-sm rounded-lg text-left transition-colors ${
          selectedCategory === "" ? "bg-blue-50 text-blue-700 font-medium" : "hover:bg-gray-50 text-gray-600"
        }`}
      >
        <span>{allLabel}</span>
      ...
      {categories.length ? (
        categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`flex items-center justify-between px-3 py-2 text-sm rounded-lg text-left transition-colors ${
                isActive ? "bg-blue-50 text-blue-700 font-medium" : "hover:bg-gray-50 text-gray-600"
              }`}
            >
              <span>{cat.name}</span>
        ...
        <p className="py-4 text-sm text-center text-gray-400">No categories found</p>
      )}
    </div>
  );
});
